...
export async function makeRegistry(world: World, updateHooks: (() => void)[]) {
      ...
      const draw = () => {
        ...
        for (let y = 0; y < portalEffectCanvas.height; y++) {
          for (let x = 0; x < portalEffectCanvas.width; x++) {
            ...
            ctx.fillStyle = `rgba(${r},${g},${b},${opacity})`; // Set fill style with adjusted opacity
  ...
  const portalFront2Texture = createPortalTexture(50, 205, 50, {
    baseOpacity: 0.05,
    opacityScale: 0.01,
    noiseScale: 0.3,
  }); // Changed RGB values to a greenish color
  const portalBack2Texture = createPortalTexture(50, 205, 50, {
    baseOpacity: 0.05,
    opacityScale: 0.01,
    noiseScale: 0.3,
  });
