...
import { getQueryUsername } from '@/src/utils/getQueryUsername';
...
export function VoxelizeProvider({
  ...
  const configRef = useRef<VoxelizeConfig | null>(null);
  ...
  useLayoutEffect(() => {
    ...
    if (queryUsername) {
      network.setUsername(queryUsername);
    }
    ...
    const toggleUI = () => {
      ...
      debug.dataWrapper.style.display = 'none';// debug.visible ? 'block' : 'none';
      gui.domElement.style.display = 'none';//debug.visible ? 'block' : 'none';
    ...
    if (isUserAdmin) {
      inputs.bind('KeyJ', toggleUI, 'in-game', {
        checkType: 'code',
      });
    }
    ...
    function createCharacter(forceOwner?: boolean, id?: string) {
    ...
    peers.createPeer = (id: string) => createCharacter(false, id);
    peers.packInfo = () => {
      const {
        x: dx,
        y: dy,
    ...
    debug.registerDisplay('Voxels to Emit', world.chunks.toEmit, 'length');
    debug.registerDisplay('Voxels to Update', world.chunks.toUpdate, 'length');
    ...
    events.on(
      'hit-player',
      (payload: { impulse: [number, number, number] }) => {
        rigidControls.body.applyImpulse(payload.impulse);
      },
    );
    ...
    if (worldName === 'main') {
      triggers.add(
        new Trigger(
          ...
          () => {
            localStorage.setItem(voxelizeWorldLocalStorageKey, 'flat2');
    ...
    async function start() {
      document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'visible') {
          ...
          clearUpdate = setWorkerInterval(update, 1000 / 60);
          isFocused = false;
        }
      });
      ...
      const update = () => {
        ...
        if (rigidControls.voxel[1] < -5) {
          ...
          rigidControls.body.velocity = [0, 0, 0];
          rigidControls.body.impulse = [0, 0, 0];
        }
      };
      ...
      // purposely not waiting
      void makeRegistry(world, updateHooks);
      ...
      gui.add(rememberedGuiData, 'canWallJump').onChange((value) => {
        config.canWallJump = value;
        localStorage.setItem(
          saveKey,
          JSON.stringify({ ...rememberedGuiData, canWallJump: value }),
        );
      });
      config.canWallJump = rememberedGuiData.canWallJump;
      ...
      gui
        .add(rememberedGuiData, 'sensitivity', 1, 200, 0.1)
        .onChange((value) => {
          rigidControls.options.sensitivity = value;
          localStorage.setItem(
            saveKey,
            JSON.stringify({ ...rememberedGuiData, sensitivity: value }),
          );
        });
      rigidControls.options.sensitivity = rememberedGuiData.sensitivity;

      gui.add(rememberedGuiData, 'fov', 30, 120, 0.1).onChange((value) => {
        camera.fov = value;
        camera.updateProjectionMatrix();
        localStorage.setItem(
          saveKey,
          JSON.stringify({ ...rememberedGuiData, fov: value }),
        );
      });
      camera.fov = rememberedGuiData.fov;
      ...
      if (isUserAdmin) {
        ...
        setTimeout(() => {
          chat.onChat({
            type: 'system',
            body: '$#FFC470$Join our discord! $http$https://discord.gg/2KnYJJh4',
          });
        }, 3000);
      }
